...
# Architecture
- backend/gestionnaire/   → Main service: JWT auth, agence/gestionnaire management (HTTP :8080, gRPC :9000)
...
- docker-compose.yml      → 4 PostgreSQL DBs + pgAdmin (root level)

Each service has its OWN Maven project, its OWN database, its OWN Flyway migrations. There is NO shared parent POM.

# Stack
- Backend: Java 21, Quarkus 3.34.1, Hibernate ORM Panache, Flyway, gRPC, SmallRye JWT
...
- Auth: JWT (RS256 — privateKey.pem + publicKey.pem in backend/gestionnaire/)
...
# Start dev
1. docker-compose up -d                          (start all 4 DBs + pgAdmin)
...
# Security — IMPORTANT
- JWT keys: backend/gestionnaire/privateKey.pem + publicKey.pem
- These files are NOT in .gitignore — add them immediately if committing
