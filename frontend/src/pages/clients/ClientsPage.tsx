...
export const clientDisplayName = (c: Client): string => {
...
export const clientInitials = (c: Client): string => {
  if (c.clientType === "PHYSICAL") {
      ...
      const payload: ClientCreateRequest = {
        ...
        ...(data.situationFamiliale && {
          situationFamiliale: data.situationFamiliale as
            | "SINGLE" | "MARRIED" | "DIVORCED" | "SEPARATED" | "WIDOWER" | "OTHER",
        }),
        ...
        ...(data.accountTypeId && { accountTypeId: Number(data.accountTypeId) }),
        ...(data.secteurActiviteId && { secteurActiviteId: Number(data.secteurActiviteId) }),
        ...(data.sousActiviteId && { sousActiviteId: Number(data.sousActiviteId) }),
        ...(user?.id && { assignedManagerId: user.id }),
        ...(data.relationAvecClient && {
          relationAvecClient: data.relationAvecClient as
            | "CLIENT" | "SUPPLIER" | "NEIGHBOUR" | "OTHER",
        }),
        ...
        >
          {filteredClients.map((c) => (
            <tr key={c.id} className="table-row-hover">
              ...
              {/* Segment */}
              <td className="px-6 py-4 whitespace-nowrap">
                <span className="text-sm text-surface-600">{c.segmentLibelle || "—"}</span>
              </td>

              {/* Business */}
              <td className="px-6 py-4 whitespace-nowrap">
                <p className="text-sm text-surface-600">{c.secteurActiviteLibelle || "—"}</p>
                <p className="text-xs text-surface-400">{c.sousActiviteLibelle || "—"}</p>
              </td>
              ...
              {/* Manager */}
              <td className="px-6 py-4 whitespace-nowrap">
                <span className="text-sm text-surface-600">{c.managerFullName || "—"}</span>
              </td>
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="flex items-center gap-1">
                  <Button variant="ghost" size="sm" onClick={() => setViewingClient(c)}>
                    View
                  </Button>
                  <Button variant="ghost" size="sm" onClick={() => setEditingClient(c)}>
                    Edit
                  </Button>
                  <Button
                    variant="ghost"
