...
const demandeSchema = z.object({
  ...
  loanPurpose: z
    .string()
    .trim()
    .min(3, "Loan purpose must have at least 3 characters"),
  requestedAmount: z.coerce
    .number()
    .gt(0, "Requested amount must be greater than 0"),
...
const clientLabel = (client: Client) => {
  if (client.clientType === "PHYSICAL") {
    return (
      `${client.firstName || ""} ${client.lastName || ""}`.trim() || client.id
    );
  }
...
const DemandeForm = ({
      ...
      ? {
          ...
          monthlyRepaymentCapacity:
            defaultValues.monthlyRepaymentCapacity != null
              ...
              : undefined,
          applicationChannel:
            defaultValues.applicationChannel?.split(":")?.[0] || "",
          applicationChannelOther: defaultValues.applicationChannel?.includes(
            "OTHER:",
          )
          ...
          legalIssueOrAccountBlocked:
            defaultValues.legalIssueOrAccountBlocked ?? false,
  ...
  const {
    fields: guaranteeFields,
    append: appendGuarantee,
    remove: removeGuarantee,
  } = useFieldArray({
    control,
    name: "guarantees",
  });
  ...
  const onFormSubmit = (data: DemandeFormData) => {
    ...
    onSubmit({
      ...
      guarantors:
        filteredGuarantors.length > 0 ? filteredGuarantors : undefined,
      guarantees:
        filteredGuarantees.length > 0 ? filteredGuarantees : undefined,
    });
  };
    ...
    <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-1">
      <div className="mb-6 rounded-lg border border-surface-200 dark:border-surface-700 bg-surface-50 dark:bg-surface-800 p-4 transition-colors">
        <label
          ...
          className="block text-label text-surface-700 dark:text-surface-300 mb-2"
        >
          {t("form.searchClient")}
        </label>
        <input
        ...
        <p className="mt-2 text-xs text-surface-600 dark:text-surface-400">
          {filteredClients.length} {t("form.clientsFound")}
        </p>
      </div>
      ...
      {selectedClient && (
        <div className="mb-4 rounded-lg bg-brand-50 dark:bg-brand-900/20 border border-brand-200 dark:border-brand-800 p-4 transition-colors">
          <p className="text-xs font-semibold text-brand-700 dark:text-brand-300 uppercase mb-2">
            <p>{t('form.selectedClient')}</p>
          </p>
      ...
      {selectedClient && (
        <div className="mb-4 rounded-xl border border-brand-200 dark:border-brand-800 bg-brand-50 dark:bg-brand-900/20 p-4 transition-colors">
         ...
         <span className="font-semibold">{t("form.firstName")}:</span>{" "}
            ...
            <p>
              <span className="font-semibold">{t("form.gender")}:</span>{" "}
              {selectedClient.gender || "—"}
            </p>

            <p>
              <span className="font-semibold">{t("form.segment")}:</span>{" "}
              {selectedClient.segmentLibelle || "—"}
            </p>

            <p>
              <span className="font-semibold">{t("form.businessSector")}:</span>{" "}
              {selectedClient.secteurActiviteLibelle || "—"}
            </p>

            <p>
              <span className="font-semibold">{t("form.businessActivity")}:</span>{" "}
              {selectedClient.sousActiviteLibelle || "—"}
            </p>

            <p>
              <span className="font-semibold">{t("form.ifcRiskLevel")}:</span>{" "}
              {selectedClient.ifcLevelOfRisk || "—"}
            </p>
          </div>
        </div>
      )}
      ...
      >
        ...
        />
        {showHint("loanPurpose", t("form.loanPurposeHint"))}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4">
        ...
        >
          <Input
            ...
            onBlur={() => setHoveredField(null)}
          />
          {showHint("requestedAmount", t("form.requestedAmountHint"))}
        </div>
        <Input
      ...
      <div className="mb-6 rounded-lg border border-rose-200 dark:border-rose-800 bg-rose-50 dark:bg-rose-900/20 p-4 transition-colors">
        <h3 className="text-sm font-semibold text-rose-800 dark:text-rose-300 mb-4">
        ...
        <div className="mb-5">
          ...
          {errors.bankingRestriction && (
            <p className="mt-1.5 text-xs text-rose-600 dark:text-rose-400">
              {errors.bankingRestriction.message}
            </p>
          )}
        </div>
        ...
        <div>
          ...
          {errors.legalIssueOrAccountBlocked && (
            <p className="mt-1.5 text-xs text-rose-600 dark:text-rose-400">
              {errors.legalIssueOrAccountBlocked.message}
            </p>
          )}
        </div>
      </div>
      ...
      <div className="mb-4 rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 p-4 transition-colors">
        <p className="text-sm font-semibold text-amber-800 dark:text-amber-300 mb-3">
          {t("form.declarationOfConsent")}
        </p>
        <div
          dir="rtl"
          className="text-sm text-surface-700 dark:text-surface-300 whitespace-pre-wrap leading-relaxed text-justify"
        >
          {CONSENT_TEXT}
        </div>
      </div>
        ...
        {guarantorFields.length > 0 ? (
          <div className="space-y-3">
            {guarantorFields.map((field, idx) => (
              <div
                key={field.id}
                className="bg-white dark:bg-surface-800 rounded-lg p-3 border border-surface-200 dark:border-surface-700 transition-colors"
              >
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  <Input
                    ...
                    error={
                      errors.guarantors?.[idx]?.clientRelationship?.message
                    }
