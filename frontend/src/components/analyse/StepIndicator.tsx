...
const StepIndicator: React.FC<StepIndicatorProps> = ({
  ...
  const effectiveActive = activeStep ?? currentStep;

  const getStepState = (stepNumber: number): 'active' | 'completed' | 'accessible' | 'locked' => {
    if (stepNumber === effectiveActive) return 'active';
  ...
  const getStepClasses = (stepNumber: number): string => {
    ...
    const base =
      'flex-1 py-3 px-2 sm:px-4 text-center text-xs sm:text-sm font-medium rounded-lg transition-all cursor-pointer';

    switch (state) {
      case 'active':
        return `${base} bg-emerald-500 text-white shadow-md`;
      case 'completed':
        return `${base} bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 cursor-pointer hover:bg-emerald-200 dark:hover:bg-emerald-900/40`;
      case 'accessible':
        return `${base} bg-surface-100 dark:bg-surface-700 text-surface-700 dark:text-surface-300 cursor-pointer hover:bg-surface-200 dark:hover:bg-surface-600`;
      case 'locked':
        return `${base} bg-surface-50 dark:bg-surface-800 text-surface-400 dark:text-surface-600 cursor-not-allowed opacity-50`;
    }
  };

  const handleStepClick = (stepNumber: number) => {
    if (getStepState(stepNumber) !== 'locked') {
  ...
  return (
    <div className="bg-white dark:bg-surface-800 rounded-lg border border-surface-200 dark:border-surface-700 p-2 sm:p-4 shadow-sm">
      ...
      <div className="sm:hidden text-center text-xs text-surface-500 dark:text-surface-400 mt-2">
        {t('step')} {effectiveActive}/{steps.length}
      </div>
    </div>
  );
};
